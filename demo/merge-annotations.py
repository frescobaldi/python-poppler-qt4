    ...
    for pg_index in range(min(target.numPages(),src.numPages())):
        ...
        for a in p_src.annotations():
            if not has_annotation(p_tgt,a):
                ...
                popplerqt4.Poppler.AnnotationUtils.storeAnnotation(a,a_el,dom)
                a_tgt = popplerqt4.Poppler.AnnotationUtils.createAnnotation(a_el)
